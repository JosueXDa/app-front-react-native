...
import { getActiveThreadsByChannel, Thread } from '@/src/entities/thread';
import { CreateThreadModal } from '@/src/features/chat/create-thread';
import { ChannelSettingsMenu } from '@/src/widgets/channel-settings';
import { ChatView } from '@/src/widgets/chat-view';
import { ThreadList } from '@/src/widgets/thread-list';
...
export function ChannelDetailsScreen() {
    ...
    const fetchChannel = async () => {
      ...
      try {
        ...
        if (selectedChannel?.id === channelId) {
          ...
          console.log(
            '[ChannelScreen] Using channel from context:',
  ...
  useEffect(() => {
    if (selectedChannel?.id === channelId) {
      console.log(
        '[ChannelScreen] Syncing channel from context:',
        selectedChannel.name,
      );
      setChannel(selectedChannel);
    }
  ...
  if (isLoading) {
    return (
      <View className="flex-1 bg-background-0 items-center justify-center">
        ...
        <Text className="text-typography-600 mt-2">Cargando canal...</Text>
      </View>
    );
  }
      ...
      {isDesktop && (
        <View className="flex-1">
          {selectedThread && channel ? (
            <ChatView channel={channel} thread={selectedThread} showBackButton={false} />
          ) : (
