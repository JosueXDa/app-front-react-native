...
import { Channel } from '../model/channel.types';
    ...
    <View className="p-4 border-b border-outline-200 bg-background-0">
      <View className="flex-row items-center mb-3">
        ...
        <View className="flex-1">
          <Text className="text-xl font-bold text-typography-900">{channel.name}</Text>
      ...
      {channel.description && (
        <Text className="text-sm text-typography-600 mb-2">{channel.description}</Text>
      )}
      ...
      {channel.category && (
        <View className="flex-row items-center">
          {listCategories.map((category, index) => (
            <View
              key={index}
              className="bg-brand-100 dark:bg-brand-900 px-2 py-1 mr-2 rounded"
            >
