...
export function ChatView({
  ...
  const processPendingQueue = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `Message ${pendingMsg.tempId} sent, server returned:`,
        createdMessage.id,
      );
      ...
      if (pendingMsg.retries < 3) {
        ...
        setMessages((prev) => prev.filter((msg) => msg.id !== pendingMsg.tempId));
      }
    } finally {
      isSendingRef.current = false;
      ...
      <View className="flex-row items-center justify-between px-4 py-3 bg-background-50 border-b border-outline-200">
        <View className="flex-row items-center">
          ...
          <View>
            ...
            <Text className="text-xs text-typography-500">{channel.name}</Text>
          </View>
        </View>
        <View className="flex-row items-center">
          <Pressable className="p-2">
            <MoreVertical size={20} color="rgb(var(--color-typography-600))" />
          </Pressable>
        </View>
      </View>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            const timeDiff = nextMessage
              ? messageTime.getTime() - new Date(nextMessage.createdAt).getTime()
              : 0;
            ...
            return (
              ...
              >
                    ...
                    <Avatar size="md" className="w-10 h-10 rounded-full">
                      {item.sender?.profile?.avatarUrl ? (
                        <AvatarImage
                          ...
                          alt={
                            item.sender.profile.displayName ||
                            item.sender.name ||
                            'User'
                          }
                        />
                ...
                <View className="flex-1">
                  ...
                  {isPending && (
                    <Text className="text-[10px] text-typography-400 mt-1">
                      Sending...
                    </Text>
                  )}
                </View>
