...
import { Thread, ThreadCard } from '@/src/entities/thread';
  ...
  if (isLoading) {
    return (
      <View className="flex-1 items-center justify-center">
        ...
        <Text className="text-typography-600 mt-2">Cargando hilos...</Text>
      </View>
    );
  }
    ...
    <View className="flex-1">
      ...
      {threads.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ThreadCard thread={item} onPress={() => onThreadPress(item)} />
          )}
